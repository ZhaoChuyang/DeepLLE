...
class VGGPerceptualLoss(torch.nn.Module):
    ...
    def forward(self, input, target, feature_layers=[0, 1, 2, 3], style_layers=[]):
        """
        Args:
            input (tensor): input image of shape (b, c, h, w), data range should be (0, 1).
            target (tensror): input image of shape (b, c, h, w), data range should be (0, 1).
        """
        if input.shape[1] != 3:
